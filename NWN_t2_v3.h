   ...
   if (IsMaster() || IsFighterRight()) {
    ...
    if ((T2_GetAltarState(WpClosestAltarLeft()) == "BATTLE" || T2_GetAltarState(WpClosestAltarRight()) == "BATTLE")
        && (sCurrentTarget == WpClosestAltarLeft() || sCurrentTarget == WpClosestAltarRight())) {
            ActionSpeakString("Defending altar, Determine Combat round!", TALKVOLUME_SHOUT);
            DetermineCombatRound();  // If the base altars are in battle and they're current goal -> fight
    } else {
        ...
        if (T2_IsOnAltar(OBJECT_SELF, sCurrentTarget, 0.4)) {
            ActionSpeakString("On enemy altar, Determine Combat round!", TALKVOLUME_SHOUT);
            DetermineCombatRound();
        } else {
           SetLocalString( OBJECT_SELF, "TARGET", sCurrentTarget);
           ClearAllActions();
           ActionSpeakString("Going to weakest point!" + sCurrentTarget, TALKVOLUME_SHOUT);
           ...
           if (GetDistanceToObject(oEnemy) < 3.0 && GetDistanceBetweenLocations(GetLocation(oEnemy), GetLocation(GetObjectByTag(sCurrentTarget))) < 1.0) {
                ActionSpeakString("Attacking Enemy who is next to my target!" + sCurrentTarget, TALKVOLUME_SHOUT);
                ActionAttack(oEnemy);
           }
        }
...
void T2_Cleric_HeartBeat() {
    ...
    while (GetIsObjectValid( oCreature ))
    {
        if (GetDistanceToObject(oCreature) < 20.0 && GetHealth(oCreature) < 4) {
            ...
            ActionSpeakString("Healing " + GetTag(oCreature), TALKVOLUME_SHOUT);
        } else {
